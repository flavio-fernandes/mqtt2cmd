        ...
        for topic, payloads in topics.items():
            for payload, h in payloads.items():
                if h not in handlers:
                    logger.warning(
                        "topic {} payload {} uses unknown msg {}".format(
                            topic, payload, h
                        )
                    )

        def get_handler_commands(
            curr_handler, curr_cmds_acc, curr_attrs_acc, curr_vars_acc
        ):
            # Likely a circular dependency issue?!?
            assert (
                len(curr_cmds_acc) < MAX_CMDS_PER_HADLER
            ), "Too manny commands for {} msg: {}".format(curr_handler, curr_cmds_acc)
            ...
            if not cmd_raw:
                ...
                assert (
                    job.get(const.CONFIG_CMD) is None
            ...
            curr_attrs_acc = dict(
                list(job_attrs.items()) + list(curr_attrs_acc.items())
            )
            ...
            for cmd in cmds:
                ...
                if cmd in handlers:
                    curr_cmds_acc, curr_attrs_acc, curr_vars_acc = get_handler_commands(
                        cmd, curr_cmds_acc, curr_attrs_acc, curr_vars_acc
                    )
                else:
